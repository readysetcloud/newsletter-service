...
|---|---|---|---|
| `robotVoice` | `formatRobotVoice`: the tinted "robot voice" card (slate left rail, emoji notch label) **plus** `<!--[if mso]>` Outlook fallbacks and Gmail degradation for the negative-margin notch labels | ✅ email variant shipped: RSC `layouts/shortcodes/robotVoice.email.html`, byte-identical output to `formatRobotVoice`; `robotVoice.html` is the web card and carries no MSO/Gmail handling | done — but the two implementations are hand-kept in sync, so a styling change to either must be ported to the other |
