...
|---|---|---|---|
| `robotVoice` | `formatRobotVoice`: the "robot voice" card — filled header bar (emoji + label), tinted body, 404 footer. Label and footer sit **inside** the card, so there are no MSO conditionals and no Gmail fallback | ✅ at parity: RSC `layouts/shortcodes/robotVoice.html` renders byte-identical HTML and serves the email output format too (no `.email.html` variant needed) | none for email-safety — but the two implementations are hand-kept in sync, so a styling change to either must be ported to the other |
...
**Net:** the email output format is mostly RSC's web shortcodes with (a) an email-safe (non-oEmbed) `social` and (b) the paragraph-spacing treatment. Reproduce both before `parse-md-to-json` is deleted, or those blocks silently break in email. `robotVoice` no longer needs Outlook/Gmail hardening: its labels moved inside the card, so one shortcode serves web and email alike.
