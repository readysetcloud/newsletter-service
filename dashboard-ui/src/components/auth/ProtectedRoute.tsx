...
export function ProtectedRoute({
  ...
  // Check tenant admin permissions if required
  if (requireTenantAdmin && !user?.isTenantAdmin && !user?.isAdmin) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-error-100">
          ...
          <h3 className="mt-2 text-sm font-medium text-foreground">Access Denied</h3>
          <p className="mt-1 text-sm text-muted-foreground">
            You don&apos;t have permission to access this page. Tenant admin access required.
          </p>
        </div>
      </div>
    );
