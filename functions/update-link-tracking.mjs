...
export const handler = async (state) => {
  ...
  while ((matches = linkRegex.exec(state.content)) !== null) {
    ...
    if (matches[1] && matches[1].indexOf('mailto:') === -1) {
      ...
      updatedContent = updatedContent.replace(matches[1], `${process.env.REDIRECT_URL}?u=${encodeURI(matches[1])}&cid=${encodeURIComponent(`${state.tenantId}_${state.issueId}`)}&s={{emailAddressHash}}`)
    }
  }
